...
// Helper function to parse 'YYYY-MM-DD' as a local Date
function parseLocalDate(dateString) {
    const [year, month, day] = dateString.split('-').map(Number);
    return new Date(year, month - 1, day);
  }
  
  // Get today's date in local time and set the time to midnight
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  // Filter messages with dates on or before today
  const pastMessages = highlightedMessages
    .filter(({ date }) => {
      const messageDate = parseLocalDate(date);
      return messageDate <= today;
    });
  
  // Sort the past messages in descending order to get the most recent one
  pastMessages.sort((a, b) => parseLocalDate(b.date) - parseLocalDate(a.date));
  
  // Select the most recent past message
  const messageToDisplay = pastMessages[0] || {
                ...
                <div>
                    ...
                    <div
                        dangerouslySetInnerHTML={{
                            __html: messageToDisplay['message']
                        }}
                    />
                    {/* Optional CTA button */}
